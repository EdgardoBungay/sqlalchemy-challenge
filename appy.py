...
def precipitation():
    ...
    query_last = session.query(Measurement.date).order_by(Measurement.date.desc()).first()

    # Get the first element of the tuple
    query_last = query_last[0]

    # Calculate the date 1 year ago from today
    # The days are equal 366 so that the first day of the year is included
    year_ago = dt.datetime.strptime(query_last, "%Y-%m-%d") - dt.timedelta(days=366)
...
def tobs(): 
    ...
    # Calculate the date 1 year ago from today
    # The days are equal 366 so that the first day of the year is included
    query_year = dt.datetime.strptime(query_last, "%Y-%m-%d") - dt.timedelta(days=366)
    # Query tobs
    results_tobs = session.query(Measurement.date, Measurement.tobs).filter(Measurement.date >= query_year).all()
